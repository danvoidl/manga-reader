<template>
  <nav class="relative w-full h-20 font-sans min-w-full mb-5 bg-main z-10 ">
    <div class="w-11/12 flex justify-between m-auto" v-if="screenWidth >= 900">
      <!--LOGO -->
      <span class="h-20 grid">
        <img
          class="h-16 self-center"
          src="data:image/png;base64,"
          alt=""
        />        
      </span>      

      <!--OPTIONS -->
      <div class="flex items-center h-20">
        <div >
          <ul class="flex text-gray-50 ">
            <li :key="option" v-for="option in navOptions" class="h-10 flex items-center cursor-pointer border-b border-transparent hover:border-b-2 hover:border-yellow-400 m-6" > {{option}} </li>            
          </ul>
        </div>

        <!-- search -->
        <div class="relative grid self-center">
          <input
            type="text"
            class="p-2 pl-8 rounded border border-gray-200 bg-gray-200 focus:bg-white focus:outline-none focus:ring-2 focus:ring-yellow-600 focus:border-transparent w-64 lg:w-64 lg:bg-gray-200 lg:border-gray-200"
            placeholder="Search..."
            value=""
          />
          <svg
            class="w-4 h-4 absolute left-2.5 top-3.5 lg:text-black"
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
            <path
              stroke-linecap="round"
              stroke-linejoin="round"
              stroke-width="2"
              d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"
            />
          </svg>
        </div>        
      </div>
    </div>

    <div v-else class="w-4.5/5 flex justify-between m-auto">
      <!--LOGO -->
      <span class="h-20 grid">
        <img
          class="h-16 ml-10 self-center"
          src="data:image/png;base64,"
          alt=""
        />
      </span>

      <span class="h-20 w-auto grid relative">
        <svg
          @click="showMenu = !showMenu"
          xmlns="http://www.w3.org/2000/svg"
          width="26"
          height="26"
          fill="white"
          class="bi bi-list self-center mr-10 cursor-pointer"
          viewBox="0 0 16 16"
        >
          <path
            fill-rule="evenodd"
            d="M2.5 12a.5.5 0 0 1 .5-.5h10a.5.5 0 0 1 0 1H3a.5.5 0 0 1-.5-.5zm0-4a.5.5 0 0 1 .5-.5h10a.5.5 0 0 1 0 1H3a.5.5 0 0 1-.5-.5zm0-4a.5.5 0 0 1 .5-.5h10a.5.5 0 0 1 0 1H3a.5.5 0 0 1-.5-.5z"
          />
        </svg>
        <div
          v-if="showMenu"
          class="absolute top-14 right-12 w-56 h-36 bg-gray-600 rounded grid"
        >
          <ul class="flex flex-col align-middle m-auto self-center">
            <li
              class="text-gray-50 mt-2 font-semibold text-sm grid self-center cursor-pointer"
              :key="option"
              v-for="option in navOptions"
            >
              {{ option }}
            </li>
          </ul>
        </div>
      </span>
    </div>
  </nav>

</template>

<script>
export default {
  props: {
    screenWidth: Number,
  },
  data() {
    return {
      navOptions: ["Home", "Mangas", "Latest", "Top Voted"],
      showMenu: false,
    };
  },
};
</script>

<style scoped>
</style>